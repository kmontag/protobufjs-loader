...
/**
 * Creates a private temporary directory and returns a function to
 * build paths inside it, plus a cleanup function which recursively
 * removes it.
 *
 * @typedef {{
...
const createTmpDir = () =>
  new Promise((resolve, reject) => {
    fs.mkdtemp(path.join(os.tmpdir(), 'protobufjs-loader-'), (err, dir) => {
      if (err) {
        reject(err);
        return;
      }
      resolve({
        ...
        cleanup: () =>
          new Promise((resolveCleanup, rejectCleanup) => {
...
const execPbts = async (resourcePath, pbtsOptions, compiledContent) => {
  ...
  // pbts expects a file path for its input. It accepts "-" for
  // stdin, but a webpack loader runs inside the build process and
  // cannot safely drive process.stdin. Write the compiled output to a
  // temp file and pass that to pbts instead.
  const tmpDir = await createTmpDir();

  try {
    const compiledFilename = tmpDir.file('compiled.js');

    // Write the compiled JS content to the temp file.
...
const loadSource = async (source, context) => {
  ...
  /**
   * Get a temp directory and write the protobuf file content into it.
   *
  ...
  try {
    /** @type { string } */
    const filename = tmpDir.file('source.proto');
